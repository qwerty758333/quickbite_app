...
export default function CheckoutScreen() {
  ...
  // Creates a local mock order number and clears the cart only after the order is placed.
  const placeOrder = () => {
    const orderId = `QB-${Math.floor(1000 + Math.random() * 9000)}`;
    clearCart();
    router.replace({ pathname: '/order-confirmation' as never, params: { orderId, pickupTime } });
  };

  return <SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.content}><Text style={styles.back} onPress={() => router.back()}>Back</Text><Text style={styles.title}>Almost there</Text><Text style={styles.muted}>Choose a pickup time and we&apos;ll have it ready.</Text><View style={styles.form}><Text style={styles.label}>Student name</Text><TextInput onChangeText={setStudentName} placeholder="Enter your name" placeholderTextColor={AppColors.textSecondary} style={styles.input} value={studentName} /><Text style={styles.label}>Pickup time</Text><TextInput onChangeText={setPickupTime} style={styles.input} value={pickupTime} /></View><View style={styles.orderBox}><Text style={styles.label}>Order summary</Text>{cartItems.map((item) => <View key={item.id} style={styles.line}><Text style={styles.muted}>{item.quantity} x {item.name}</Text><Text style={styles.muted}>${(item.price * item.quantity).toFixed(2)}</Text></View>)}<View style={styles.line}><Text style={styles.totalLabel}>Total</Text><Text style={styles.total}>${cartSubtotal.toFixed(2)}</Text></View></View><PrimaryButton disabled={!studentName.trim() || cartItems.length === 0} label="Place order" onPress={placeOrder} /></ScrollView></SafeAreaView>;
}
